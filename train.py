...
def train(env_name, print_things=True, train_run_id=0, train_episodes=500):
    # Create a Gym environment
    # env = gym.make(env_name)
    env = simulator.Robot()

    # Get dimensionalities of actions and observations
    action_space_dim = 4 #env.action_space.shape[-1]
    observation_space_dim = 4*3+4*3+3 #env.observation_space.shape[-1]
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            observation, reward, done = env.step(action.detach().numpy())
    ...
    data = pd.DataFrame({"episode": np.arange(len(reward_history)),
                         ...
                         # Change algorithm name for plots
...
#    env = gym.make(env_name)
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            observation, reward, done = env.step(action.detach().cpu().numpy())
    ...
    print("Episode goal: ", env.goal, "Tool position:", env.get_robot_position()[-1]['xyz'])
    env.plot()
